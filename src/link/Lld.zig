...
fn elfLink(lld: *Lld, arena: Allocator) !void {
            ...
            target.cpu.arch == .xtensa))
    {
        ...
        if (elf.z_max_page_size) |size| {
            ...
            // Mirrors clang/lib/Driver/ToolChains/Linux.cpp: 32-bit ARM gets 4k, AArch64 and
            // x86_64 get 16k (Android 15+ 16k pages); everything else keeps LLD's default.
            switch (target.cpu.arch) {
                .arm, .armeb, .thumb, .thumbeb => {
                    try argv.append("-z");
                    try argv.append("max-page-size=4096");
                },
                .aarch64, .aarch64_be, .x86_64 => {
                    try argv.append("-z");
                    try argv.append("max-page-size=16384");
                },
                else => {},
            }
        }
            ...
            if (comp.config.link_libc) {
                if (comp.libc_installation != null) {
                    ...
                    for (bionic.libs) |lib| {
                        ...
                        const lib_path = try arena.print("{f}{c}lib{s}.so", .{
                            comp.bionic_so_files.?.dir_path, fs.path.sep, lib.name,
                        });
                        try argv.append(lib_path);
                    }
                } else {

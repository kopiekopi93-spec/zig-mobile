...
const bionic = @import("libs/bionic.zig");
...
bionic_so_files: ?bionic.BuiltSharedObjects = null,
...
const QueuedJobs = struct {
    ...
    bionic_crt_file: [@typeInfo(bionic.CrtFile).@"enum".field_names.len]bool = @splat(false),
    ...
    bionic_shared_objects: bool = false,
                ...
                if (target.isDarwinLibC()) {
                    ...
                    switch (comp.config.output_mode) {
                        ...
                        .Lib => switch (comp.config.link_mode) {
                            .static => {},
                            .dynamic => {
                                comp.queued_jobs.bionic_crt_file[@backingInt(bionic.CrtFile.crtbegin_so_o)] = true;
                                comp.queued_jobs.bionic_crt_file[@backingInt(bionic.CrtFile.crtend_so_o)] = true;
                            },
                        },
                        .Exe => switch (comp.config.link_mode) {
                            .dynamic => {
                                comp.queued_jobs.bionic_crt_file[@backingInt(bionic.CrtFile.crtbegin_dynamic_o)] = true;
                                comp.queued_jobs.bionic_crt_file[@backingInt(bionic.CrtFile.crtend_android_o)] = true;
                            },
                            .static => {
                                comp.queued_jobs.bionic_crt_file[@backingInt(bionic.CrtFile.crtbegin_static_o)] = true;
                                comp.queued_jobs.bionic_crt_file[@backingInt(bionic.CrtFile.crtend_android_o)] = true;
                            },
                        },
                    }

                    comp.queued_jobs.bionic_shared_objects = true;
...
pub fn destroy(comp: *Compilation) void {
    ...
    if (comp.bionic_so_files) |*bionic_file| {
        bionic_file.deinit(gpa, io);
    }
...
fn dispatchPrelinkWork(comp: *Compilation, main_progress_node: std.Progress.Node) void {
    ...
    if (comp.queued_jobs.bionic_shared_objects) {
        prelink_group.async(io, buildBionicSharedObjects, .{ comp, main_progress_node });
    }
    ...
    for (0..@typeInfo(bionic.CrtFile).@"enum".field_names.len) |i| {
        if (comp.queued_jobs.bionic_crt_file[i]) {
            const tag: bionic.CrtFile = @fromBackingInt(@intCast(i));
            prelink_group.async(io, buildBionicCrtFile, .{ comp, tag, main_progress_node });
        }
    }
...
fn buildBionicCrtFile(comp: *Compilation, crt_file: bionic.CrtFile, prog_node: std.Progress.Node) void {
    if (bionic.buildCrtFile(comp, crt_file, prog_node)) |_| {
        comp.queued_jobs.bionic_crt_file[@backingInt(crt_file)] = false;
    } else |err| switch (err) {
        error.AlreadyReported => return,
        else => comp.lockAndSetMiscFailure(.bionic_crt_file, "unable to build Bionic {s}: {s}", .{
            @tagName(crt_file), @errorName(err),
        }),
    }
}

fn buildBionicSharedObjects(comp: *Compilation, prog_node: std.Progress.Node) void {
    if (bionic.buildSharedObjects(comp, prog_node)) |_| {
        comp.queued_jobs.bionic_shared_objects = false;
    } else |err| switch (err) {
        error.AlreadyReported => return,
        else => comp.lockAndSetMiscFailure(.bionic_shared_objects, "unable to build Bionic libc shared objects: {t}", .{err}),
    }
}
        ...
        if (ext != .ll and ext != .bc) {
            ...
            if (comp.config.link_libc) {
                if (target.abi.isAndroid()) {
                    ...
                    try argv.append(try std.fmt.allocPrint(arena, "-D__ANDROID_API__={d}", .{api_level}));
                    try argv.append("-D__ANDROID__");
                } else if (target.isGnuLibC()) {

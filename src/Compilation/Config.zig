    ...
    const link_mode = b: {
        ...
        if (explicitly_exe_or_dyn_lib and link_libc and
            (target.requiresLibC() or
                ...
                ((target.isGnuLibC() or target.isFreeBSDLibC() or target.isNetBSDLibC() or target.isOpenBSDLibC() or target.abi.isAndroid()) and
        ...
        if (explicitly_exe_or_dyn_lib and link_libc) {
            ...
            if (target.isFreeBSDLibC() or target.isNetBSDLibC() or target.isOpenBSDLibC() or target.abi.isAndroid()) {
                break :b .dynamic;
            }
        }

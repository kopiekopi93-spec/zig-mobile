...
pub const available_libcs = [_]ArchOsAbi{
    ...
    .{ .arch = .arm, .os = .linux, .abi = .androideabi },
    ...
    .{ .arch = .aarch64, .os = .ios, .abi = .none, .os_ver = .{ .major = 14, .minor = 0, .patch = 0 } },
    .{ .arch = .aarch64, .os = .ios, .abi = .simulator, .os_ver = .{ .major = 14, .minor = 0, .patch = 0 } },
    .{ .arch = .aarch64, .os = .linux, .abi = .android },
    ...
    .{ .arch = .x86_64, .os = .linux, .abi = .android },
    ...
    .{ .arch = .x86_64, .os = .ios, .abi = .simulator, .os_ver = .{ .major = 14, .minor = 0, .patch = 0 } },
...
/// Android API levels for which Bionic libc data is bundled (see lib/libc/bionic).
pub const android_api_min: u32 = 21;
pub const android_api_max: u32 = 37;
    ...
    for (available_libcs) |libc| {
        if (target.cpu.arch == libc.arch and target.os.tag == libc.os and target.abi == libc.abi) {
            if (target.abi.isAndroid()) {
                ...
                if (api < android_api_min or api > android_api_max) return false;
            }
...
pub fn isLibCLibName(target: *const std.Target, name: []const u8) bool {
    ...
    if (target.abi.isGnu() or target.abi.isMusl() or target.abi.isAndroid()) {
        if (eqlIgnoreCase(ignore_case, name, "m"))
            return true;
    ...
    if (target.abi.isAndroid()) {
        if (eqlIgnoreCase(ignore_case, name, "android"))
            return true;
        if (eqlIgnoreCase(ignore_case, name, "binder_ndk"))
            return true;
        if (eqlIgnoreCase(ignore_case, name, "aaudio"))
            return true;
        if (eqlIgnoreCase(ignore_case, name, "mediandk"))
            return true;
        if (eqlIgnoreCase(ignore_case, name, "EGL"))
            return true;
        if (eqlIgnoreCase(ignore_case, name, "amidi"))
            return true;
        // Note: "stdc++" is deliberately NOT included here — it's handled by
        // isLibCxxLibName below, and isLibCLibName is checked first by callers
        // (see std.Build.Module.linkSystemLibrary), so including it here would
        // shadow that check and set link_libc instead of link_libcpp.
        if (eqlIgnoreCase(ignore_case, name, "OpenMAXAL"))
            return true;
        if (eqlIgnoreCase(ignore_case, name, "z"))
            return true;
        if (eqlIgnoreCase(ignore_case, name, "vulkan"))
            return true;
        if (eqlIgnoreCase(ignore_case, name, "sync"))
            return true;
        if (eqlIgnoreCase(ignore_case, name, "OpenSLES"))
            return true;
        if (eqlIgnoreCase(ignore_case, name, "jnigraphics"))
            return true;
        if (eqlIgnoreCase(ignore_case, name, "neuralnetworks"))
            return true;
        if (eqlIgnoreCase(ignore_case, name, "GLESv3"))
            return true;
        if (eqlIgnoreCase(ignore_case, name, "log"))
            return true;
        if (eqlIgnoreCase(ignore_case, name, "GLESv1_CM"))
            return true;
        if (eqlIgnoreCase(ignore_case, name, "nativehelper"))
            return true;
        if (eqlIgnoreCase(ignore_case, name, "GLESv2"))
            return true;
        if (eqlIgnoreCase(ignore_case, name, "camera2ndk"))
            return true;
        if (eqlIgnoreCase(ignore_case, name, "nativewindow"))
            return true;
        if (eqlIgnoreCase(ignore_case, name, "icu"))
            return true;
    }
